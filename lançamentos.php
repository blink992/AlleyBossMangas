...
if(isset($_SESSION['ID'])) {
    ...
    while($manga = $query->fetch_assoc()) {
        $mangas_favoritados[] = $manga['favoritos'];
    }
...
while($manga = $mangas->fetch_array()) {
...
$array_lançamentos_in_fav = array();


?>

<div id="lançamentos-in-fav" style="visibility:collapse;height:0px;border-radius: 45px;border-bottom-left-radius:15px;border-top-right-radius:15px;border: 4px outset rgb(143, 157, 189); background-color:rgb(83, 89, 102);display:flex;flex-direction:column;width:98%;align-self:center;position:relative">
    <h5 id="lançamentos-in-fav-title" style="margin-bottom:10px;align-self:center" class="basic-text">Novidades dos seus mangás favoritos</h5>
    <div style="height: max-content;display:flex;flex-direction:column;" class="lançamento-in-fav"></div>
    <span class="recebe_dados"></span>
    <i id="btn-expand-list-lançamentos-in-fav" onclick="expand_list('recebe_dados.php')" style="order:20;margin:18px;cursor: pointer;color:rgba(146, 163, 197, 1);font-size:165%;align-self:center;position:relative;width:94%" class="center fa-solid fa-chevron-down"></i>
</div>

<?php foreach($array_mangas_lançamentos as $manga_id) { ?>
    
<?php 
    
    
    if($index == (count($array_mangas_lançamentos) - 1)){

        if($array_lançamentos_in_fav){

            ?><script>

                var lançamentos = document.getElementById("lançamentos-in-fav");
        lançamentos.style.visibility = "visible";
        lançamentos.style.paddingTop = "2px";
        lançamentos.style.paddingBottom = "8px";
        lançamentos.style.marginTop = "22px";
        lançamentos.style.height = "max-content";
        
        var btn = document.getElementById('btn-expand-list-lançamentos-in-fav');

        var qtd_lançamentos_inf_fav = <?= count($array_lançamentos_in_fav) ?>;

        var count_1 = 10;

        if(qtd_lançamentos_inf_fav < count_1) {
        ...
        btn.onclick = function(){
            ...
            var dados = {
                array_lançamentos_in_fav : '<?= serialize($array_lançamentos_in_fav) ?>',
                index_reset : count_1
        }
        $.post("recebe_dados.php", dados, function(retorna){
            
            $(".lançamento-in-fav").html(retorna);
        });        
        };
    ...
    if(date('m') == $data_diretorio) {
        if(in_array($manga['ID'], $mangas_favoritados)) {
            $array_lançamentos_in_fav[] = $manga['ID'];
        }
        else {
            if($index < 15){
?>

<!-- collection-item de mangás -->
                ...
                <div style="position:absolute; left:0px;display:flex; flex-direction:column;width:100%;height:100%">
                    ...
                    <div style="position:relative" class="container-titulo-lista-manga">
                        <form style="width: 100%; height: 100%;overflow-x:hidden" action="pagina_manga.php" method="POST">
                            <button type="int" name="dados" value="<?= $manga['ID'] ?>" style="cursor:pointer;border-style: none;padding: 0px;width: 100%; height: 100%;" class="transparent"><title style="height:max-content;overflow-x:hidden;" class="titulo-manga"><?php echo $manga["Titulo"] ?></title></button>
                        </form>
                    </div>
            ...
            <div class="sinopse-e-infos">
                ...
                <div class="infos-lançamentos hoverable">
                    ...
                    <div style="position:relative;order:5;margin-left:0px;left: 0px;" class="pontuacao-manga">
                        ...
                        <text style="font-weight: 600;transform:scale(0.85);color:rgb(137, 199, 199);align-self: center;margin-left: 8px;font-size: inherit;"><?php echo number_format($manga["Nota"], 2, ',', '.') ?></text>
                    </div>
                        <!-- Pontuação do mangá  -->
                    <div id="visualizacoes_pagina_navbar" style="position:relative;order:1;margin-left:0px;left: 0px;" class="pontuacao-manga">
                        ...
                        <text style="font-weight: 600;transform:scale(0.85);color:rgb(137, 199, 199);align-self: center;margin-left: 8px;font-size: inherit;"><?php echo number_format($manga["Visualizacoes"], 0, ',', '.') ?></text>
                    </div>
...
<div id="expanded_list" style="order:15;display: flex;flex-direction:column;justify-content:space-between;position:relative" class="expanded_lançamentos">
                            
</div>

<i id="btn-expand-list" onclick="expand_list('expanded_lançamentos.php')" style="order:20;margin:18px;cursor: pointer;color:rgba(146, 163, 197, 1);font-size:165%;align-self:center;position:relative;width:94%" class="center fa-solid fa-chevron-down"></i>
