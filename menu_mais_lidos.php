<?php 
    $query = "SELECT * FROM mangas ORDER BY Visualizacoes ASC";
    $mais_lidos = $mysqli->query($query) or die($mysqli->error);
?>
<?php while($manga = $mais_lidos->fetch_array()) : ?>
        ...
        <div class="painel-manga-ranqueado card">
            ...
            <div class="container-titulo-menus-direita">
                <form style="width: 100%; height: 100%;" action="pagina_manga.php" method="POST">
                    <button  type="int" name="dados" value="<?= $manga['ID'] ?>" style="border-style: none;padding: 0px;" class="transparent"><title style="cursor:pointer;text-align:justify" class="titulo-manga-ranqueado left"><?php echo $manga["Titulo"] ?></title></button>
                </form>
            </div>
