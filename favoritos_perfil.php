...
<div class="painel-favoritos-perfil">
<h4 style="align-self:center" class="basic-text">Favoritos</h4>
<?php
$sql_code = "SELECT * FROM $username WHERE favoritos <> 'NULL'";
    ...
    <!-- Título da seção mangás ranquados -->
       ...
       <div id="fixed-btn-pagina-navbar" style="z-index:4;transform:scale(0.86);top:-10px;right:4px" class="floating-btn-pagina-navbar">
            <a class="btn-large btn-floating">
                <i style="color:inherit" onclick="showOptionsManga(<?= $manga['ID'] ?>, 'menu_mais_lidos')" class="fa-sharp fa-solid fa-ellipsis"></i>
            </a>

            <div id="opcoes_manga<?= $manga['ID']?>menu_mais_lidos" style="width:max-content;height:58px;right:60px;top:37px;display:flex;flex-direction:row;z-index:-5;opacity:0;position:absolute;transition: opacity 0.5s, top 0.5s, transform 0.8s;transform: scale(0.5)" >
               
                <!-- BOTÃO DE FAVORITAR MANGÁ  -->
                <div style="list-style: none;margin-left:12px">
                    <a onclick="addFavoritos(<?= $manga['ID'] ?>, 'menu_mais_lidos')" style="background-color: rgb(18, 45, 70)" id="favoritar-pagina-manga" class="btn btn-floating hoverable">
                        <i id="icon_favorite<?= $manga['ID'] ?>menu_mais_lidos" class="material-icons">
                        <?php        
                                if(!isset($_SESSION)){
                                    session_start();
                                }
        ...
        <div class="painel-manga-ranqueado card">
            ...
            <div class="sinopse-e-infos-menus-direita">
                ...
                <!-- Pontuação do mangá  -->
					...
					<div id="visualizações_favoritos_perfil" style="position:relative;order:1;margin-left:0px;left: 0px;font-size: 120%;" class="pontuacao-manga">
...
if(lenght_screen < 350){
    var elemento = document.getElementById("visualizações_favoritos_perfil");
...
<?php } ?>
<div id="expanded_list" style="display: flex;flex-direction:column;justify-content:space-between;position:relative" class="expanded_favoritos_perfil">
</div>
<i id="btn-expand-list" onclick="expand_list('expanded_favoritos_perfil.php')" style="margin:18px;cursor: pointer;color:rgba(146, 163, 197, 1);font-size:165%;align-self:center;position:relative;width:98%" class="center fa-solid fa-chevron-down"></i>
</div>
...
var count = 15;
function expand_list(pagina) {
    count = count + 15;
    var dados = {
        index_reset : count
    }
    $.post(pagina, dados, function(retorna){
        $(".expanded").html(retorna);
      });
